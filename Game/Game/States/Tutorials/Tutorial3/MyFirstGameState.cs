namespace Dive.Game.States.Tutorials.Tutorial3
{
    ...
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(MyFirstGameState));
        ...
        public override void Start(IGameState previous)
        {
            ...
            TaskInfo myTask = new TaskInfo()
            {
                ...
                Task = () =>
                {
                    ...
                    GameEngine.Instance.StateManager.ChangeState<MySecondGameState>();
                }
            };

            GameEngine.Instance.Scheduler.ScheduleTask(myTask);
        }
